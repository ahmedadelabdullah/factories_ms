											...
											<tbody>
												...
												<tr class="cloning-row" id={{$loop->index}}>
													...
													<td class="tx-right">
														...
														<option value="{{$product->product_name}}" {{$item->product_name == $product->product_name ? 'selected' : ''}}>{{$product->product_name}}</option>
														@endforeach
														</select>
														@error('quantity')
                                                        <span>{{$message}}
...
{{--  الخصم وهو حاصل ضرب عدد القطع فى مبلغ الخصم للوحدة --}}
				...
				<tr>
					...
					<td class="tx-right" colspan="2">
						{{-- المبلغ المستحق --}}
						<input type="number" name="total_due"  value="{{$invoice->total_due_return}}" id="total_due" class="form-control" readonly>
					</td>
				</tr>
											</tfoot>
		</table>
